...
### **Bước 0: Kiểm Tra Thông Tin Người Dùng**

**File:** `backend/chatbots/chatbot_check_info.py`

Trước khi xử lý, hệ thống kiểm tra:
- Câu hỏi có liên quan đến tư vấn thuốc không
- Có đủ thông tin để tư vấn không (triệu chứng, bệnh, v.v.)
- Có thể sử dụng lịch sử hội thoại để bổ sung ngữ cảnh không

**Kết quả:**
- Nếu không liên quan hoặc không đủ thông tin → Trả về thông báo yêu cầu thêm thông tin
...
**Lưu ý:** Task này chạy trong background, không block luồng chính để tối ưu thời gian phản hồi.

#### **b) Lấy Dữ Liệu Bộ Nhớ Dài Hạn**
**File:** `backend/get_doc/get_long_term.py`

- Đọc thông tin người dùng từ `long_term_memory.txt`
- Cung cấp context cá nhân hóa cho câu trả lời
- Bao gồm: dị ứng, bệnh lý, tiền sử, v.v.

#### **c) Mở Rộng Câu Hỏi (Query Expansion)**
**File:** `backend/get_doc/expand_query.py`

Sử dụng LLM để mở rộng câu hỏi gốc thành 2 câu hỏi:
- **Câu 1**: Giữ nguyên câu hỏi gốc
- **Câu 2**: Câu hỏi mở rộng về thành phần/công dụng/liều lượng/tác dụng phụ

**Mục đích:** Tăng độ chính xác khi tìm kiếm tài liệu trong vector database
...
**Files:** `backend/get_doc/get_docs.py` + `backend/get_doc/rag.py`
...
project/
├── backend/
│   ├── chatbot_main.py              # Orchestrator chính - điều phối toàn bộ luồng
│   ├── tracing.py                   # OpenTelemetry setup cho observability
│   ├── chatbots/
│   │   ├── chatbot_check_info.py    # Kiểm tra thông tin người dùng
│   │   ├── chatbot_summary.py       # Tóm tắt tài liệu dài
│   │   └── chatbot_update_ltm.py    # Cập nhật bộ nhớ dài hạn
│   ├── get_doc/
│   │   ├── expand_query.py          # Mở rộng câu hỏi
│   │   ├── get_docs.py              # Lấy documents từ vector DB
│   │   ├── get_long_term.py         # Đọc Long-Term Memory
│   │   └── rag.py                   # RAG với Qdrant
│   └── memory/
│       └── long_term_memory.txt     # Lưu trữ LTM (thông tin người dùng)
│
├── frontend/
│   └── gradio.ipynb                 # Giao diện Gradio ChatInterface
│
├── drugs-data-main/
│   ├── data/
│   │   ├── details/                 # Dữ liệu thuốc (JSON files)
│   │   │   ├── Cơ-xương-khớp/
│   │   │   ├── Thuốc-giảm-đau-hạ-sốt-kháng-viêm/
│   │   │   └── ... (các danh mục thuốc khác)
│   │   └── urls/                    # URLs nguồn dữ liệu
│   ├── main.py                      # Scraper dữ liệu thuốc
│   └── utils.py                     # Utilities
│
├── embedding.py                     # Tạo embeddings từ dữ liệu thuốc
├── embedding_to_qdrant.py           # Upload embeddings lên Qdrant
├── delete_qdrant.py                 # Xóa dữ liệu trên Qdrant (utility)
